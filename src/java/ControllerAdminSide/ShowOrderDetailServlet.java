...
public class ShowOrderDetailServlet extends HttpServlet {
    ...
    throws ServletException, IOException {
        ...
        try {
            ...
            OrderDTO order = d.getOrderById(orderId);
            List<OrderMealDTO> meals = d.getOrderMealDetails(orderId);

            if (order != null) {
                request.setAttribute("OrderDetail", order);
                request.setAttribute("MealDetails", meals);
            } else {
                request.setAttribute("message", "Order not found.");
            }
        } catch (Exception e) {
            e.printStackTrace();
            request.setAttribute("message", "Error retrieving order details.");
        } finally {
