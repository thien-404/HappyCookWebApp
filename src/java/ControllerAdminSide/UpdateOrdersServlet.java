...
public class UpdateOrdersServlet extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            boolean rs = d.UpdateOrders(order);
            if (rs) {
                request.setAttribute("success", "Udate success");
            } else {
                request.setAttribute("message", "Udate fail");
            }
        } catch (Exception e) {
            e.printStackTrace();
        } finally {
            request.getRequestDispatcher("MangegeOrdersServlet").forward(request, response);
        }
    }
